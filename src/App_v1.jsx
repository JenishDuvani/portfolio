...
/**
 * Jenish — MCA Portfolio (from-scratch, dark OS-inspired, clean & elegant)
 * - Single file so it runs in this sandbox.
 * - Black base + tasteful gradients (pink/fuchsia accents).
 * - Glassy navbar w/ active underline; Bento projects; Timelines; Badges; Callouts.
 * - No external UI deps beyond react-icons to avoid CDN issues.
 * - Contact form: POSTs to configurable endpoint (AWS-friendly). In this preview it just simulates success.
 */

// -----------------------------
// Configurable content (fill later)
// -----------------------------
const PROFILE = {
  name: "Jenish Duvani",
...
function Badge({ children }) {
  return (
    <span className="inline-flex items-center rounded-full border border-pink-500/30 bg-pink-500/10 px-3 py-1 text-xs text-pink-300">
      {children}
    </span>
  );
}
...
function Card({ children, className = "" }) {
  return (
    <div className={`relative rounded-2xl border border-white/10 bg-white/5 backdrop-blur px-5 py-4 ${className}`}>
      {children}
    </div>
  );
}
    ...
    <div className={`${dark ? "bg-black text-white" : "bg-white text-black"} relative min-h-screen scroll-smooth`}>
      ...
      <header className="sticky top-4 z-50 mx-auto w-full max-w-6xl px-6">
        <nav className="flex items-center justify-between rounded-2xl border border-white/10 bg-white/5 px-4 py-3 backdrop-blur">
          ...
          <ul className="hidden gap-5 md:flex">
            {navIds.map((id) => (
              <li key={id}>
                <a href={`#${id}`} className={`relative pb-1 text-xs uppercase tracking-wider text-zinc-400 transition hover:text-zinc-100 ${active === id ? "text-zinc-100" : ""}`}>
                  {id}
          ...
          <button onClick={() => setDark(!dark)} className="rounded-lg p-2 text-zinc-300 hover:bg-white/10">
            {dark ? <FiSun /> : <FiMoon />}
          </button>
        </nav>
      </header>

      <section id="home" className="mx-auto flex min-h-[78vh] w-full max-w-6xl flex-col items-center justify-center px-6 text-center">
        ...
        <h1 className="text-balance bg-gradient-to-r from-zinc-50 via-zinc-200 to-zinc-50 bg-clip-text text-5xl font-extrabold leading-tight text-transparent md:text-6xl">
          {PROFILE.name}
        </h1>
        <p className="mt-2 text-lg text-zinc-400">{PROFILE.title}</p>
        <p className="mt-3 max-w-2xl text-zinc-400">{PROFILE.elevator}</p>
        <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
          <a href="#projects" className="inline-flex items-center rounded-xl bg-gradient-to-r from-pink-500 to-fuchsia-600 px-5 py-2 text-sm font-medium text-white shadow hover:scale-[1.02]">
            View Work <FiChevronRight className="ml-2" />
          </a>
          <a href="#resume" className="inline-flex items-center rounded-xl border border-white/10 bg-white/[0.06] px-5 py-2 text-sm text-zinc-100 hover:border-pink-500/40 hover:text-pink-300">
            <FiDownload className="mr-2" /> Resume
          </a>
          <a href={PROFILE.socials.github} target="_blank" rel="noreferrer" className="rounded-xl border border-white/10 bg-white/[0.06] px-4 py-2 text-sm text-zinc-100 hover:border-pink-500/40 hover:text-pink-300">
            <FiGithub />
          </a>
          <a href={PROFILE.socials.linkedin} target="_blank" rel="noreferrer" className="rounded-xl border border-white/10 bg-white/[0.06] px-4 py-2 text-sm text-zinc-100 hover:border-pink-500/40 hover:text-pink-300">
            <FiLinkedin />
          </a>
        </div>
        <div className="mt-10 grid w-full max-w-4xl grid-cols-3 gap-3">
      ...
      <Section id="resume" title="Resume" subtitle="Grab a copy or request via email">
        <div className="flex flex-wrap items-center gap-3">
          <a href="#" className="inline-flex items-center rounded-xl bg-gradient-to-r from-pink-500 to-fuchsia-600 px-5 py-2 text-sm font-medium text-white shadow hover:scale-[1.02]">
            <FiDownload className="mr-2" /> Download PDF
          </a>
          <a href={`mailto:${PROFILE.email}`} className="inline-flex items-center rounded-xl border border-white/10 bg-white/[0.06] px-5 py-2 text-sm text-zinc-100 hover:border-pink-500/40 hover:text-pink-300">
      ...
      <Section id="contact" title="Contact" subtitle="Send a message — wired for AWS later">
        <form onSubmit={onSubmit} className="grid gap-4 md:grid-cols-2">
          ...
          <div className="md:col-span-2">
            <button className="inline-flex items-center rounded-xl bg-gradient-to-r from-pink-500 to-fuchsia-600 px-5 py-2 text-sm font-medium text-white shadow hover:scale-[1.02]">
              Send
            </button>
